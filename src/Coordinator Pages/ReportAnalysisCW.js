...
function  CoReportAnalysismain() {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (showExportMenu && !event.target.closest('.co-ras-print-button-container')) {
    ...
    <>
      ...
      <div className="co-layout">
        ...
        <div className="co-main-content" style={{marginLeft:'295px'}}>
          <div className="co-ras-filter-box">
            <div className="co-ras-tab-container">
              {/* Note: The 'Round wise' tab label is kept but its functionality is now removed */}
              <div className="co-ras-tab-inactive" onClick={() => navigate(`/report-analysismain`)}>Round wise Analysis</div>
              <div className="co-ras-tab-active"  >Company wise Analysis</div>
              <div className="co-ras-tab-inactive" onClick={() => navigate(`/report-analysissw`)}>Student wise Analysis</div>
            </div>

            <div className="co-ras-filter-inputs">
              {/* Company Filter */}
              <select 
                className="co-ras-filter-select-1" 
                value={companyFilter} 
              ...
              {/* Batch Filter */}
              <select 
                className="co-ras-filter-select-2"
                value={batchFilter} 
              ...
              <DatePicker
                ...
                className="co-ras-filter-date-input"
                isClearable={true}
              />
        ...
        {/* 🗑️ REMOVED: co-ras-round-buttons-container block is removed */}
          ...
          <div className="co-ras-table-section">
            <div className="co-ras-table-header-row">
              <div className="co-ras-table-title">COMPANY ANALYSIS REPORT</div>
              <div className="co-ras-table-actions">
                <button 
                  className={statusFilter === 'Passed' ? "co-ras-view-status-btn-filtered" : "co-ras-view-status-btn-all"}
                  onClick={toggleStatusFilter}
                >
                  {statusFilter === 'Passed' ? 'View All Students' : 'View Passed Students'}
                </button>
                
                <div className="co-ras-print-button-container">
                  <button 
                    className="co-ras-print-btn" 
                  ...
                  {showExportMenu && (
                    <div className="co-ras-export-menu">
            ...
            <table className="co-ras-data-table">
              <thead>
                <tr>
              ...
              <div className="co-ras-table-body-scroll" style={{maxHeight:"301px"}}>
                <tbody>
                  ...
                  {filteredData.map((student, index) => (
                    <tr key={index}>
                      ...
                      <td>
                        <div className={student.Status === 'Passed' ? "co-ras-status-cell-passed" : "co-ras-status-cell-rejected"}>
                          {student.Status}
                        </div>
                      </td>
