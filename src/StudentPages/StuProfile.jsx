...
function StuProfile({ onLogout, onViewChange }) {
    ...
    }, [normalizeText, studentApplications]);
    ...
    }, [normalizeDate, normalizeText, studentAttendanceRecords]);

    const getDriveStatus = useCallback((application, attendanceAbsent = false) => {
        if (attendanceAbsent) return 'Absent';

        const normalizedStatus = normalizeText(application?.status);
        if (normalizedStatus === 'absent') return 'Absent';
        if (normalizedStatus === 'rejected' || normalizedStatus === 'failed') return 'Rejected';
        if (normalizedStatus === 'placed' || normalizedStatus === 'selected') return 'Placed';

        const rounds = application?.rounds || [];
        if (rounds.length === 0) return 'Pending';

        if (rounds.some((round) => round?.status === 'Absent')) return 'Absent';
        if (rounds.some((round) => round?.status === 'Failed')) return 'Rejected';

        const passedCount = rounds.filter((round) => round?.status === 'Passed').length;
        if (passedCount > 0) return `Passed-${passedCount}`;

        return 'Pending';
    }, [normalizeText]);

    const syncCompanyRealtimeData = useCallback(async (student) => {
        if (!student?._id) return;

        try {
            const [eligibleResponse, appsResponse, attendanceResponse] = await Promise.all([
                mongoDBService.getEligibleStudentsForStudent(student._id),
                mongoDBService.getStudentApplications(student._id),
                fetchStudentAttendanceRecords(student)
            ]);

            const drives = eligibleResponse?.drives || [];
            setEligibleDrives(Array.isArray(drives) ? drives : []);
    ...
    useEffect(() => {
        ...
        syncCompanyRealtimeData(activeStudent);

        const refreshFromStorage = () => {
            try {
                const latest = JSON.parse(localStorage.getItem('studentData') || 'null');
                if (latest?._id) {
                    syncCompanyRealtimeData(latest);
                }
            } catch {
        ...
        const refreshInterval = setInterval(refreshFromStorage, 30000);
    ...
    const driveAnalytics = useMemo(() => {
        ...
        eligibleDrives.forEach((drive) => {
            ...
            const overallStatus = getDriveStatus(application, attendanceAbsent);
            ...
            if (overallStatus === 'Placed') {
                ...
                pendingCount += 1;
                pendingDriveCandidates.push({
                    companyName,
                    date: normalizeDate(drive?.driveStartDate || drive?.companyDriveDate || drive?.driveEndDate)
                });
            }
            ...
            const plannedRounds = Array.isArray(drive?.roundDetails)
                ? drive.roundDetails.map((item) => (item || '').toString().trim()).filter(Boolean)
                : [];
            ...
            if (plannedRounds.length > 0) {
                plannedRounds.forEach((plannedRoundName, index) => {
                    ...
                    const matchedRound = rounds.find((round) =>
                        normalizeText(round?.name || round?.roundName || round?.roundType || round?.type) === normalizeText(plannedRoundName) ||
                        Number(round?.roundNumber) === index + 1
                    );
        ...
        const pieData = orderedBuckets.map((bucket, index) => ({
            name: bucket.name,
            value: totalAttempted > 0 ? Math.round((bucket.attempted / totalAttempted) * 100) : 0,
        ...
        const roundDetails = orderedBuckets.reduce((acc, bucket) => {
            ...
            if (bucket.attempted === 0) {
                ...
                if (passRate >= 60) {
                    good.push(`Healthy conversion rate at ${passRate}% for this category.`);
                } else {
                    bad.push(`Conversion is ${passRate}%. Focus improvement in this category.`);
                }

                if (bucket.absent > 0) {
                    bad.push(`${bucket.absent} round(s) marked absent. Attendance impact detected.`);
                }

                if (bucket.passed > 0) {
                    good.push(`${bucket.passed} round(s) cleared successfully.`);
                }
            }

            acc[bucket.name] = {
                companies: Array.from(bucket.companies.entries()).map(([name, status]) => ({ name, status })),
                good,
                bad
            };
        ...
        const bestAt = sortable
            .slice()
            .sort((a, b) => b.passRate - a.passRate)
            .slice(0, 3)
            .map((item) => item.category);

        const workOn = sortable
            .slice()
        ...
        const upcomingDrive = pendingDriveCandidates
            ...
            .find((item) => !item.date || item.date >= today) || pendingDriveCandidates[0];

        const stepFlags = [
            eligibleDrives.length > 0,
            totalAttempted > 0,
            shortlistedCount > 0,
            placedCount > 0 || rejectedCount > 0
        ];

        const currentStepIndex = stepFlags.findIndex((flag) => !flag);

        const progressSteps = ['Applied', 'Attended', 'Interview', 'Result'].map((label, index) => {
            if (currentStepIndex === -1) return { label, s: 'done' };
            if (index < currentStepIndex) return { label, s: 'done' };
            if (index === currentStepIndex) return { label, s: 'current' };
            return { label, s: 'pending' };
        });

        return {
            totalCompaniesAttended: companyNames.size,
    ...
    }, [eligibleDrives, findApplicationForDrive, getDriveStatus, isAttendanceAbsentForDrive, normalizeDate, normalizeText]);
    ...
    const companyStats = useMemo(() => ({
        ...
        preferredModeOfDrive: studentData?.preferredModeOfDrive || 'N/A',
        lastDriveAttended: driveAnalytics.lastDriveAttended || 'N/A',
        lastDriveResult: driveAnalytics.lastDriveResult || 'Pending',
    ...
    const computedCompanyMetrics = useMemo(() => ({
        totalCompaniesAttended: Number(driveAnalytics.totalCompaniesAttended || 0),
        totalDrivesAttended: Number(driveAnalytics.totalDrivesAttended || 0),
        shortlistedCount: Number(driveAnalytics.shortlistedCount || 0),
        lastDriveAttended: (driveAnalytics.lastDriveAttended || '').toString().trim(),
        lastDriveResult: (driveAnalytics.lastDriveResult || '').toString().trim(),
        highestPackageDrive: Number(driveAnalytics.highestPackageLpa || 0),
        totalRoundsCleared: Number(driveAnalytics.totalRoundsCleared || 0)
    }), [
        driveAnalytics.highestPackageLpa,
        driveAnalytics.lastDriveAttended,
        driveAnalytics.lastDriveResult,
        driveAnalytics.shortlistedCount,
        driveAnalytics.totalCompaniesAttended,
        driveAnalytics.totalDrivesAttended,
        driveAnalytics.totalRoundsCleared
    ]);

    useEffect(() => {
        return () => {
            if (companyAutoSaveTimerRef.current) {
                clearTimeout(companyAutoSaveTimerRef.current);
            }
        };
    }, []);
    ...
    useEffect(() => {
        ...
        const existingMetrics = {
            totalCompaniesAttended: Number(studentData?.totalCompaniesAttended || 0),
            totalDrivesAttended: Number(studentData?.totalDrivesAttended || 0),
            shortlistedCount: Number(studentData?.shortlistedCount || 0),
            lastDriveAttended: (studentData?.lastDriveAttended || '').toString().trim(),
            lastDriveResult: (studentData?.lastDriveResult || '').toString().trim(),
            highestPackageDrive: Number(studentData?.highestPackageDrive || 0),
            totalRoundsCleared: Number(studentData?.totalRoundsCleared || 0)
        };

        const nextKey = JSON.stringify(computedCompanyMetrics);
        const existingKey = JSON.stringify(existingMetrics);

        if (nextKey === existingKey || nextKey === companyAutoSaveKeyRef.current) {
            return;
        }
        ...
        companyAutoSaveTimerRef.current = setTimeout(async () => {
            if (companyAutoSaveInFlightRef.current) return;
            companyAutoSaveInFlightRef.current = true;

            try {
                await fastDataService.updateProfile(studentId, computedCompanyMetrics);
                companyAutoSaveKeyRef.current = nextKey;
            } catch (error) {
                console.error('Silent company metrics sync failed:', error);
            } finally {
                companyAutoSaveInFlightRef.current = false;
            }
        }, 1200);
    }, [
        computedCompanyMetrics,
        studentData?._id,
        studentData?.highestPackageDrive,
        studentData?.id,
        studentData?.lastDriveAttended,
        studentData?.lastDriveResult,
        studentData?.shortlistedCount,
        studentData?.totalCompaniesAttended,
        studentData?.totalDrivesAttended,
        studentData?.totalRoundsCleared
    ]);
    ...
    const trainingCardEntries = useMemo(() => {
        ...
        // Never render schedule-driven placeholders for students with no assignment/attendance data.
    ...
    const isStudentPlaced = useMemo(() => {
        const placementCandidates = [
            ...
            studentData?.placement,
            studentData?.placementResult,
            studentData?.finalPlacementStatus,
            studentData?.companyPlacementStatus,
            studentData?.jobStatus,
            studentData?.status
        ]
            .map((value) => normalizeText(value))
            .filter(Boolean);

        const hasPlaced = placementCandidates.some((value) =>
            value === 'placed' ||
            value === 'selected' ||
        ...
        const hasNotPlaced = placementCandidates.some((value) =>
            value === 'not placed' ||
            value === 'unplaced' ||
            value === 'notplaced' ||
            value === 'rejected' ||
        ...
        if (hasNotPlaced) return false;

        if (typeof studentData?.isPlaced === 'boolean') {
            return studentData.isPlaced;
        }

        return driveAnalytics.placedCount > 0;
    }, [driveAnalytics.placedCount, normalizeText, studentData]);
    ...
    useEffect(() => {
        if (!hasCompanyData && showAnalysis) {
