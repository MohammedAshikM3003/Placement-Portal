...
function MainContent({ onViewChange }) {
  ...
  const linkedinUrl = useMemo(() => {
    if (!studentData) return 'N/A';
    let val = studentData.linkedinLink || studentData.linkedin || studentData.linkedIn || studentData.linkedinUrl || studentData.linkedInUrl || studentData.linkedinURL || studentData.linkedInURL || studentData.personalInfo?.linkedinLink || studentData.personalInfo?.linkedin;
    if (!val && (studentData._id || studentData.id)) {
      const studentId = studentData._id || studentData.id;
      try {
        const stored = localStorage.getItem(`resumeBuilderData_${studentId}`) || localStorage.getItem('resumeBuilderData');
        if (stored) {
          const parsed = JSON.parse(stored);
          val = parsed.personalInfo?.linkedinLink || parsed.personalInfo?.linkedin || parsed.personalInfo?.linkedIn || parsed.linkedinLink || parsed.linkedin;
        }
      } catch (e) { }
    }
  ...
  const githubUrl = useMemo(() => {
    if (!studentData) return 'N/A';
    let val = studentData.githubLink || studentData.github || studentData.gitHub || studentData.githubUrl || studentData.gitHubUrl || studentData.githubURL || studentData.gitHubURL || studentData.githubProfile || studentData.personalInfo?.githubLink || studentData.personalInfo?.github;
    if (!val && (studentData._id || studentData.id)) {
      const studentId = studentData._id || studentData.id;
      try {
        const stored = localStorage.getItem(`resumeBuilderData_${studentId}`) || localStorage.getItem('resumeBuilderData');
        if (stored) {
          const parsed = JSON.parse(stored);
          val = parsed.personalInfo?.githubLink || parsed.personalInfo?.github || parsed.personalInfo?.gitHub || parsed.githubLink || parsed.github;
        }
      } catch (e) { }
    }
    return (val && typeof val === 'string' && val.trim()) ? val.trim() : 'N/A';
  ...
  return (
    <Box sx={{ p: 0, flex: 1, display: 'flex', flexDirection: 'column', width: '100%', minHeight: 0 }}>
      <Box display="flex" gap={2} sx={{ width: '100%', flex: 1, alignItems: 'stretch', flexDirection: { xs: 'column', md: 'row' } }}>
          ...
          >
            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', mt: 0, flexGrow: 1 }}>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.8, width: '100%', flexGrow: 1, mb: 1, px: 1 }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', width: '100%', gap: 0.5 }}><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', minWidth: '75px' }}>Name</Typography><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', minWidth: '15px', textAlign: 'center' }}>:</Typography><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', flex: 1, wordBreak: 'break-word' }}>{studentData ? `${studentData.firstName} ${studentData.lastName}` : 'N/A'}</Typography></Box>
                <Box sx={{ display: 'flex', alignItems: 'center', width: '100%', gap: 0.5 }}><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', minWidth: '75px' }}>DOB</Typography><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', minWidth: '15px', textAlign: 'center' }}>:</Typography><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', flex: 1, wordBreak: 'break-word' }}>{formatDob(studentData?.dob)}</Typography></Box>
                <Box sx={{ display: 'flex', alignItems: 'center', width: '100%', gap: 0.5 }}><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', minWidth: '75px' }}>Degree</Typography><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', minWidth: '15px', textAlign: 'center' }}>:</Typography><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', flex: 1, wordBreak: 'break-word' }}>{degreeDisplay}</Typography></Box>
                <Box sx={{ display: 'flex', alignItems: 'center', width: '100%', gap: 0.5 }}><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', minWidth: '75px' }}>Branch</Typography><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', minWidth: '15px', textAlign: 'center' }}>:</Typography><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', flex: 1, wordBreak: 'break-word' }}>{studentData?.branch || 'N/A'}</Typography></Box>
                <Box sx={{ display: 'flex', alignItems: 'center', width: '100%', gap: 0.5 }}><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', minWidth: '75px' }}>Year</Typography><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', minWidth: '15px', textAlign: 'center' }}>:</Typography><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', flex: 1, wordBreak: 'break-word' }}>{studentData?.currentYear || 'N/A'}</Typography></Box>
                <Box sx={{ display: 'flex', alignItems: 'center', width: '100%', gap: 0.5 }}><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', minWidth: '75px' }}>Sem</Typography><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', minWidth: '15px', textAlign: 'center' }}>:</Typography><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', flex: 1, wordBreak: 'break-word' }}>{studentData?.currentSemester || 'N/A'}</Typography></Box>
                <Box sx={{ display: 'flex', alignItems: 'center', width: '100%', gap: 0.5 }}><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', minWidth: '75px' }}>CGPA</Typography><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', minWidth: '15px', textAlign: 'center' }}>:</Typography><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', flex: 1, wordBreak: 'break-word' }}>{studentData?.overallCGPA || 'Null'}</Typography></Box>
                <Box sx={{ display: 'flex', alignItems: 'center', width: '100%', gap: 0.5 }}><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', minWidth: '75px' }}>Mobile</Typography><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', minWidth: '15px', textAlign: 'center' }}>:</Typography><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', flex: 1, wordBreak: 'break-word' }}>{studentData?.mobileNo || 'N/A'}</Typography></Box>
                <Box sx={{ display: 'flex', alignItems: 'center', width: '100%', gap: 0.5 }}><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', minWidth: '75px' }}>Mail ID</Typography><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', minWidth: '15px', textAlign: 'center' }}>:</Typography><Typography variant="body1" color="#e3f2fd" fontSize="14px" sx={{ fontWeight: 'bold', flex: 1, wordBreak: 'break-all' }}>{studentData?.primaryEmail || studentData?.domainEmail || 'N/A'}</Typography></Box>
                <Box sx={{ display: 'flex', alignItems: 'center', width: '100%', gap: 0.5 }}><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', minWidth: '75px' }}>LinkedIn</Typography><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', minWidth: '15px', textAlign: 'center' }}>:</Typography><Typography variant="body1" color="#e3f2fd" fontSize="14px" sx={{ fontWeight: 'bold', flex: 1, wordBreak: 'break-all' }}>{linkedinUrl}</Typography></Box>
                <Box sx={{ display: 'flex', alignItems: 'center', width: '100%', gap: 0.5 }}><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', minWidth: '75px' }}>GitHub</Typography><Typography variant="body1" color="#e3f2fd" fontSize="15px" sx={{ fontWeight: 'bold', minWidth: '15px', textAlign: 'center' }}>:</Typography><Typography variant="body1" color="#e3f2fd" fontSize="14px" sx={{ fontWeight: 'bold', flex: 1, wordBreak: 'break-all' }}>{githubUrl}</Typography></Box>
              </Box>
            </Box>
            <Box sx={{ width: '100%', px: 1, pb: 0, mt: 'auto' }}>
              <Button
                fullWidth
                variant="contained"
        ...
        {/* Right Column */}
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, width: { xs: '100%', md: '70%' }, flex: 1, height: '100%' }}>
          {isCheckingResume ? (
            /* Loading State */
            <Box sx={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: '300px' }}>
            ...
            <>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', flex: 1, height: '100%' }}>
                <Paper elevation={3} sx={{
                  width: '100%', height: '100%', flex: 1, p: { xs: 2.5, md: 3.5 },
                  background: 'linear-gradient(135deg, #dbeafe 0%, #bfdbfe 100%)',
                  borderRadius: '16px',
                  boxShadow: '0 4px 12px rgba(0,0,0,0.08)',
                  ...
                  <Typography variant="h4" fontWeight={800} color="#1e3a5f" mb={1} fontSize="28px" align="center">Build Your Professional Resume</Typography>
                  <Typography fontSize="16.5px" color="#334155" mb={2.5} align="center" sx={{ lineHeight: 1.5, maxWidth: '650px', fontWeight: 500 }}>
                    Build a recruiter-ready, ATS-optimized resume in minutes. Showcase your top skills and achievements to stand out to top employers.
                  </Typography>

                  {/* Step-by-step instructions for new students */}
                  {!resumeFromDB && (
                    <Box sx={{ width: '100%', my: 'auto', py: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                      <Typography fontWeight={700} fontSize="17px" color="#1e3a5f" mb={1.5} align="center">
                      ...
                      <Box sx={{
                        ...
                        {[
                          { step: '1', icon: <TouchApp sx={{ fontSize: 30, color: '#2563eb' }} />, title: 'Click "Build Resume"', desc: 'Click the button below to open the resume builder form.' },
                          { step: '2', icon: <EditNote sx={{ fontSize: 30, color: '#2563eb' }} />, title: 'Fill Your Details', desc: 'Enter your personal info, education, skills & projects.' },
                          { step: '3', icon: <SaveAlt sx={{ fontSize: 30, color: '#2563eb' }} />, title: 'Save & Generate', desc: 'Submit the form and your resume PDF is created instantly.' },
                          { step: '4', icon: <CloudDownload sx={{ fontSize: 30, color: '#2563eb' }} />, title: 'Preview & Download', desc: 'View, download, or check your ATS compatibility score.' },
                        ].map((item) => (
                          <Paper key={item.step} elevation={2} sx={{
                            ...
                            <Box mb={0.8}>{item.icon}</Box>
                            <Typography fontWeight={700} fontSize="14px" color="#1e3a5f" mb={0.6} lineHeight={1.3}>
                              {item.title}
                            </Typography>
                            <Typography fontSize="12.5px" color="#64748b" lineHeight={1.4}>
                              {item.desc}
                            </Typography>
                          </Paper>
                        ))}
                      </Box>
                      <Typography fontSize="13px" color="#ef4444" mt={1.5} align="center" fontWeight={600} fontStyle="italic">
