...
function AdminRARW() {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      // UPDATED CLASS: Admin-rarw-print-button-container
      if (showExportMenu && !event.target.closest(`.${styles['Admin-rarw-print-button-container']}`)) {
  ...
  // The filtering logic now depends only on the APPLIED filter states, status filter, and selected round
  useEffect(() => {
    ...
    // Filter by selected round - show students who reached this round or beyond
    if (selectedRound) {
      const selectedRoundNum = parseInt(selectedRound.split(' ')[1]);
      currentFilteredData = currentFilteredData.filter(student => {
        const studentRoundNum = parseInt(student.Rounds.split(' ')[1]);
        return studentRoundNum >= selectedRoundNum;
      });
    }

    setFilteredData(currentFilteredData);

  }, [companyFilter, batchFilter, startDateFilter, endDateFilter, statusFilter, selectedRound]); 
  ...
  const rounds = Array.from({ length: 12 }, (_, i) => `Round ${i + 1}`);
        ...
        <div className={styles['Admin-rarw-main-content']}>
          ...
          <div className={styles['Admin-rarw-filter-box']}>
            ...
            <div className={styles['Admin-rarw-tab-container']}>
              ...
              <div className={styles['Admin-rarw-tab-inactive']}>Company wise <br/> Analysis</div>
              <div className={styles['Admin-rarw-tab-inactive']}>Department wise <br/>Analysis</div>
              <div className={styles['Admin-rarw-tab-inactive']}>Student wise<br/> Analysis</div>
            </div>
            ...
            <div className={styles['Admin-rarw-filter-inputs']}>
              <select className={styles['Admin-rarw-filter-select']} value={companyFilter} onChange={(e) => setCompanyFilter(e.target.value)}>
                {uniqueCompanies.map(company => (<option key={company} value={company}>{company}</option>))}
              </select>
              <select className={styles['Admin-rarw-filter-select']} value={batchFilter} onChange={(e) => setBatchFilter(e.target.value)}>
              ...
              <DatePicker 
                ...
                className={styles['Admin-rarw-filter-date-input']} 
                isClearable={true}
              />
              <DatePicker 
          ...
          <div className={styles['Admin-rarw-table-section']}>
            {/* UPDATED CLASSES for header row, title, and actions */}
            <div className={styles['Admin-rarw-table-header-row']}>
              <div className={styles['Admin-rarw-table-title']}>STUDENT WISE ANALYSIS</div>
              <div className={styles['Admin-rarw-table-actions']}>
                <button className={statusFilter === 'Passed' ? styles['Admin-rarw-view-status-btn-filtered'] : styles['Admin-rarw-view-status-btn-all']} onClick={toggleStatusFilter}>
                  {statusFilter === 'Passed' ? 'View All' : 'View status'}
                </button>
                <div className={styles['Admin-rarw-print-button-container']}>
                  <button className={styles['Admin-rarw-print-btn-green']} onClick={handlePrintClick}>Print</button>
                  {showExportMenu && (
                ...
                <tbody>
                  {filteredData.map((student, index) => (
                    <tr key={index}>
                      ...
                      <td>
                        <div className={student.Status === 'Passed' ? styles['Admin-rarw-status-cell-placed-green'] : styles['Admin-rarw-status-cell-rejected']}>
