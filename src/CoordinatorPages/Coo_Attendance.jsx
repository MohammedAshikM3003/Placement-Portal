...
export default function Attendance({ onLogout, currentView, onViewChange }) {
  ...
  // Unique company options for the first dropdown
  const companyDropdownOptions = useMemo(() => {
    const seen = new Set();
    return groupedDrivesArray
      .filter(g => {
        if (seen.has(g.companyName)) return false;
        seen.add(g.companyName);
        return true;
      })
      .map(g => ({ label: g.companyName, value: g.companyName }));
  }, [groupedDrivesArray]);

  // Job role options filtered by the selected company
  const jobRoleDropdownOptions = useMemo(() => {
    const source = selectedCompany
      ? groupedDrivesArray.filter(g => g.companyName === selectedCompany)
      : groupedDrivesArray;
    const seen = new Set();
    return source
      .filter(g => {
        if (seen.has(g.jobRole)) return false;
        seen.add(g.jobRole);
        return true;
      })
      .map(g => ({ label: g.jobRole, value: g.jobRole }));
  }, [groupedDrivesArray, selectedCompany]);

  // Handle company selection — clear job role + students
  ...
  // Handle job role selection — find the matching group and load it
  const handleJobRoleSelect = (jobRole) => {
    setSelectedJobRole(jobRole);
    const group = groupedDrivesArray.find(
      g => g.companyName === selectedCompany && g.jobRole === jobRole
    );
      ...
      <div className={styles["co-at-layout-main"]}>
        ...
        <div className={styles["co-at-main-content-layout"]}>
          ...
          <div className={styles["co-at-filter-section"]}>
            ...
            {/* 2) Select Job Role */}
            <Dropdown
              options={jobRoleDropdownOptions}
            ...
            {/* 4) Select End Date (auto-filled, read-only) */}
            <Dropdown
            ...
            <button
              type="button"
              className={styles['co-at-clear-btn']}
              onClick={handleClearFilters}
              disabled={!selectedCompany && !selectedJobRole && !startDate && !endDate}
            >
              Clear
            </button>
          </div>
