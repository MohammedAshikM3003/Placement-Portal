      ...
      <div className="achievements-table-container">
        <div className="table-header">
          ...
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
            <button className="refresh-btn" onClick={refreshAchievements} style={{ display: 'none' }}>
...
function TableRow({ id, no, year, semester, section, comp, date, prize, status, selected, onSelect, fileName, fileData, onViewFile, onDownloadFile, achievements }) {
  ...
  const displayDate = date ? date.split('-').reverse().join('-') : 'Loading...';
  ...
  return (
    <tr>
      ...
      <td data-label="Year">{year || 'Loading...'}</td>
      <td data-label="Semester">{semester || 'Loading...'}</td>
      <td data-label="Competition">{comp || 'Loading...'}</td>
      {/* MODIFIED: Use the formatted displayDate variable */}
      <td data-label="Date">{displayDate}</td>
      <td data-label="Prize">{prize || 'Loading...'}</td>
      <td data-label="Status"><span className={statusClass}>{status?.charAt(0).toUpperCase() + status?.slice(1) || 'Loading...'}</span></td>
