...
import { triggerOtpOrExecute } from './utils/mailServiceUtils';
...
function MainRegistration() {
  ...
  const scrollToFieldAndBlink = useCallback((fieldName) => {
    ...
    } else if (fieldName === 'skillSet' || fieldName === 'skills') {
      groupContainer = sectionRefs.skills?.current || formRef.current.querySelector('[data-field-group="skills"]');
      field = groupContainer;
    } else {
    ...
    if (field || groupContainer) {
      ...
      setTimeout(() => {
        if (wrapperToBlink) {
          ...
          const container = groupContainer || sectionRefs.skills?.current;
          if (container) {
            container.classList.add('field-blink');
            const firstInteractive = container.querySelector('button, input');
      ...
      const sectionMap = {
        ...
        quota: 'other', firstGraduate: 'other', skillSet: 'skills',
  ...
  const handleSave = useCallback(
    async (event) => {
      ...
      try {
        ...
        setRegistrationEmail(studentData.domainEmail || studentData.email || studentData.primaryEmail);

        await triggerOtpOrExecute({
          onSkipOtp: async () => {
            await handleOtpSuccess(studentData);
          },
          onOpenOtp: () => {
            setIsOtpOpen(true);
          }
  ...
  const handleOtpSuccess = useCallback(
    async (overrideData = null) => {
      setIsRegistering(true);
      try {
        const studentData = overrideData || pendingStudentData;
        ...
        <div className={cx("mr-main")}>
          ...
          <div className={cx("mr-dashboard-area")} id="mr-dashboard-area">
            <form ref={formRef} onSubmit={handleSave} onChange={handleInputChange}>
              ...
              {/* ── Core Skills ── */}
              <div className={cx("mr-profile-section-container")} ref={sectionRefs.skills} data-field-group="skills">
