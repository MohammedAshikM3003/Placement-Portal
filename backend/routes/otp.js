...
const { sendMail, getMailServiceEnabled, EMAIL_EVENTS } = require('../services/mail/mailService');
...
router.post('/send', async (req, res) => {
    ...
    if (!getMailServiceEnabled()) {
        console.log(`[OTP] Mail service is currently DISABLED. Bypassing email send for ${normalizedEmail}.`);
        ...
        if (isProd) {
            return res.json({
                ...
                message: 'Mail service is disabled by administrator. Verification skipped.',
                maskedEmail: maskEmail(normalizedEmail)
            });
        }

        // Development mode helper code
        const dummyOtpVal = '123456';
        const hashedOtp = await bcrypt.hash(dummyOtpVal, 10);
        ...
        if (existingOtp) {
            ...
            await new Otp({ email: normalizedEmail, hashedOtp, purpose, role, expiresAt }).save();
        }
        ...
        return res.json({
            ...
            disabled: true,
            devMode: true,
            message: 'Mail service disabled. Development code 123456 auto-filled.',
            maskedEmail: maskEmail(normalizedEmail)
        });
    }
    ...
    try {
        ...
        const eventType = purpose === 'admin_transfer' ? EMAIL_EVENTS.ADMIN_PROFILE_TRANSFER_OTP : EMAIL_EVENTS.OTP_VERIFICATION;
        console.log(`[OTP] Generating verification code for recipient role ${role} (Event: ${eventType})`);
        await sendMail({
            eventType,
            to: normalizedEmail,
            role,
