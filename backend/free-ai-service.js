...
class FreeResumeAnalysisService {
    ...
    try {
      ...
      if (cleanText.length < 50) {
        ...
        const keywords = [
          ...
          'mohammed', 'ashik', 'resume', 'cv', 'curriculum'
        ];
  ...
  detectName(text) {
    const namePatterns = [
      ...
      /(?:mohammed|ashik|john|smith|david|miller|alex|james|michael|robert)/i
    ];
  ...
  getEnhancedFallbackAnalysis(fileData, fileName) {
    ...
    const hasName = /(name|mohammed|ashik|john|smith)/i.test(extractedText);
