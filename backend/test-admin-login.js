...
async function testAdminLogin() {
    ...
    try {
        ...
        const response1 = await axios.post(`${API_URL}/api/auth/admin-login`, {
        ...
        console.log(`   Admin: ${response1.data.admin?.fullName}\n`);

        // Test 2: Second login (should be cached)
        console.log('Test 2: Second login (cached)...');
        const start2 = Date.now();
        const response2 = await axios.post(`${API_URL}/api/auth/admin-login`, {
            adminLoginID: 'admin1000',
            adminPassword: 'admin1000'
        });
        ...
        await axios.post(`${API_URL}/api/auth/admin-login`, {
            adminLoginID: 'admin1000',
            adminPassword: 'admin1000'
        });
        ...
        if (error.response?.status === 404) {
            console.log('\n💡 TIP: Admin account does not exist. Run:');
